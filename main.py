...
class Bar(tk.Frame):
	def __init__(self, master, bg, wd: int) -> None:
		self.master = master
...
class MainActivity(tk.Tk):
	...
	def drawBars(self) -> None:
		...
		for colour in self.colours:
			hex = "#{0:02x}{1:02x}{2:02x}".format(*colour)
			Bar(self.frameBars, hex, wd=(WIDTH/self.shownWD))
		# self.frameBars.pack_propagate(0)

	def sort(self) -> None:
		if not self.random: return
